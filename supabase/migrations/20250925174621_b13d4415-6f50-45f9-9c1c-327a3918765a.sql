...
-- Create a new policy that only allows authenticated admin users to read subscriber data
...
-- Also fix other ml_* tables that have similar public access issues
...
-- Create proper admin-only policies for all MailerLite tables
...
-- Add INSERT/UPDATE/DELETE policies for service role (edge functions)
